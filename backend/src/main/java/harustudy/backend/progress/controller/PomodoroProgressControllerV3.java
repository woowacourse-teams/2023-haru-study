...
public class PomodoroProgressControllerV3 {
    ...
    public ResponseEntity<PomodoroProgressResponseV3> findProgress(@PathVariable("studyId") Long studyId,
    ...
    @PostMapping("/api/v3/studies/{studyId}/progresses/{progressId}/next-step")
    ...
    public ResponseEntity<Void> participate(@PathVariable("studyId") Long studyId,
            @RequestBody PomodoroProgressRequestV3 request) {
        return ResponseEntity.created(URI.create("/api/v3/studies/" + studyId + "/progresses/" + 1L)).build();
    }
}
