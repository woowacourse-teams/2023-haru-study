...
class ParticipantTest {
    ...
    void 멤버의_닉네임이_1자_미만_10자_초과이면_예외를_던진다(String nickname) {
        // given, when, then
        assertThatThrownBy(() -> Participant.of(study, member, nickname))
    ...
    void 멤버의_닉네임이_1자_이상_10자_이하이면_정상_케이스이다(String nickname) {
        // given, when, then
        assertThatCode(() -> Participant.of(study, member, nickname))
    ...
    void 닉네임이_동일한지_확인할_수_있다() {
        ...
        Participant otherParticipant = Participant.of(study, member, "nickname");
    ...
    void 닉네임이_다른지_확인할_수_있다() {
        // given
        Participant participant = Participant.of(study, member, "nickname");
        Participant otherParticipant = Participant.of(study, member, "another");
    ...
    void 스터디에_첫_번째로_참여하면_방장이다() {
        // given, when
        Participant host = Participant.of(study, member, "host");
    ...
    void 스터디에_첫_번째_이후로_참여하면_방장이_아니다() {
        // given, when
        Participant.of(study, member, "host");
        Participant participant = Participant.of(study, member, "parti");
