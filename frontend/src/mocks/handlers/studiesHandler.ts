...
export const studiesHandler = [
  // 필터링 조건으로 스터디 조회 API
  rest.get(`${API_BASE_URL}/studies`, (req, res, ctx) => {
  ...
  // 스터디 생성 API
  rest.post(`${API_BASE_URL}/studies`, (req, res, ctx) => {
    const requestAuthToken = req.headers.get('Authorization')?.split(' ')[1];
    ...
    return res(
      ctx.status(201),
      ctx.set({ 'Content-Type': 'application/json', Location: `${API_BASE_URL}/studies/1` }),
      ctx.delay(1000),
    );
  }),

  // 다음 스터디 단계로 이동 API
  rest.post(`${API_BASE_URL}/studies/:studyId/next-step`, (req, res, ctx) => {
  ...
  // 단일 스터디 정보 조회 API
  rest.get(`${API_BASE_URL}/studies/:studyId`, (req, res, ctx) => {
